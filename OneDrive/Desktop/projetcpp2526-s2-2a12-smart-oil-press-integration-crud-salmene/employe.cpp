...
bool resolveFingerprintColumn(QString *outColumn, QString *errorMessage)
{
    if (!outColumn) {
        if (errorMessage) {
            *errorMessage = "Parametre de sortie invalide pour la colonne d'empreinte.";
        }
        return false;
    }
    ...
    if (!query.exec(
            ...
            "AND COLUMN_NAME IN ('FINGERPRINT_ID', 'FINGERPRINT', 'EMPREINTE_ID', 'EMPREINTE')")) {
        if (errorMessage) {
            *errorMessage = "Impossible de verifier la colonne d'empreinte : " + query.lastError().text();
        }
        return false;
    }
    ...
    while (query.next()) {
        availableColumns << query.value(0).toString().trimmed().toUpper();
    }

    for (const QString &candidate : preferredOrder) {
    ...
    if (errorMessage) {
        *errorMessage = "Aucune colonne d'empreinte trouvee dans EMPLOYE. Ajoutez FINGERPRINT_ID.";
    }
    return false;
}
...
bool EmployeRepository::authenticateByFingerprintId(
    int fingerprintId,
    QString *outIdentity,
    QString *outRole,
    int *outEmployeId,
    QString *errorMessage)
{
    if (fingerprintId <= 0) {
        if (errorMessage) {
            *errorMessage = "Identifiant d'empreinte invalide.";
        }
        return false;
    }

    QString fingerprintColumn;
    if (!resolveFingerprintColumn(&fingerprintColumn, errorMessage)) {
        return false;
    }

    QSqlQuery query;
    query.prepare(
        QString("SELECT ID_EMPLOYE, NOM, PRENOM, POSTE FROM EMPLOYE WHERE %1 = :fingerprint_id")
            .arg(fingerprintColumn));
    ...
    if (!query.next()) {
        if (errorMessage) {
            *errorMessage = "Aucun employe n'est associe a cette empreinte.";
        }
        return false;
    }

    const int employeId = query.value("ID_EMPLOYE").toInt();
    const QString identity = query.value("NOM").toString().trimmed() + " " + query.value("PRENOM").toString().trimmed();
    const QString role = query.value("POSTE").toString().trimmed();

    if (query.next()) {
        if (errorMessage) {
            *errorMessage = "Cette empreinte est associee a plusieurs employes.";
        }
        return false;
    }

    if (outIdentity) {
        *outIdentity = identity.simplified();
    }
    if (outRole) {
        *outRole = role;
    }
    if (outEmployeId) {
        *outEmployeId = employeId;
    }
    return true;
}

bool EmployeRepository::getEmployeIdentityById(
    int idEmploye,
    QString *outIdentity,
    QString *errorMessage)
{
    ...
    if (outIdentity) {
        *outIdentity = (query.value("NOM").toString().trimmed() + " " + query.value("PRENOM").toString().trimmed()).simplified();
    }
    return true;
}

bool EmployeRepository::assignFingerprintId(
    int idEmploye,
    int fingerprintId,
    QString *outIdentity,
    QString *errorMessage)
{
    if (idEmploye <= 0) {
        if (errorMessage) {
            *errorMessage = "Veuillez choisir un employe valide.";
        }
        return false;
    }
    ...
    if (fingerprintId <= 0) {
        if (errorMessage) {
            *errorMessage = "L'ID de l'empreinte doit etre strictement positif.";
        }
        return false;
    }

    QString identity;
    if (!getEmployeIdentityById(idEmploye, &identity, errorMessage)) {
        return false;
    }
    ...
    checkQuery.prepare(
        QString("SELECT ID_EMPLOYE FROM EMPLOYE WHERE %1 = :fingerprint_id AND ID_EMPLOYE <> :id")
            .arg(fingerprintColumn));
    checkQuery.bindValue(":fingerprint_id", fingerprintId);
    checkQuery.bindValue(":id", idEmploye);

    if (!checkQuery.exec()) {
        if (errorMessage) {
            *errorMessage = "Erreur SQL: " + checkQuery.lastError().text();
        }
        return false;
    }
    ...
    if (checkQuery.next()) {
        if (errorMessage) {
            *errorMessage = "Cet ID d'empreinte est deja attribue a un autre employe.";
        }
        return false;
    }
    ...
    updateQuery.prepare(
        QString("UPDATE EMPLOYE SET %1 = :fingerprint_id WHERE ID_EMPLOYE = :id")
            .arg(fingerprintColumn));
    updateQuery.bindValue(":fingerprint_id", fingerprintId);
    updateQuery.bindValue(":id", idEmploye);

    if (!updateQuery.exec()) {
        if (errorMessage) {
            *errorMessage = "Erreur SQL: " + updateQuery.lastError().text();
        }
        return false;
    }
    ...
    if (updateQuery.numRowsAffected() <= 0) {
        if (errorMessage) {
            *errorMessage = "Aucun employe n'a ete mis a jour.";
        }
        return false;
    }
