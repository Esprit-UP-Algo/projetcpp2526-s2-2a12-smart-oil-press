...
QString MachineWidget::resolveMachineTable()
{
    ...
    const QStringList allTables = db.tables(QSql::Tables);

    const QStringList candidates = {
        "MACHINE", "MACHINES", "SMARTOIL.MACHINE", "SMARTOIL.MACHINES",
        "SMARTOIL\".\"MACHINE", "SMARTOIL\".\"MACHINES"
    };

    auto normalize = [](const QString &name) {
    ...
    // 1) direct table list from driver
    for (const QString &candidate : candidates) {
        const QString normalizedCandidate = normalize(candidate).replace(".", "");
        for (const QString &actual : allTables) {
            QString normalizedActual = normalize(actual);
            if (normalizedActual == normalize(candidate) || normalizedActual == normalizedCandidate) {
                qDebug() << "Machine table resolved by list:" << actual;
                return actual;
            }

            // If actual includes schema prefix
            if (normalizedActual.endsWith(normalize(candidate))) {
                qDebug() << "Machine table resolved by suffix:" << actual;
                return actual;
            }
        }
    }

    // 2) try probe queries with standard candidates
    for (const QString &candidate : candidates) {
        QSqlQuery probe;
        if (probe.exec(QString("SELECT * FROM %1 WHERE 1=0").arg(candidate))) {
            qDebug() << "Machine table resolved by probe:" << candidate;
            return candidate;
        }
    }

    // 3) fallback: inspect all table names for any that contain MACHINE
    for (const QString &ct : allTables) {
        if (normalize(ct).contains("MACHINE")) {
            qDebug() << "Machine table resolved by contains:" << ct;
            return ct;
        }
    }

    qDebug() << "Machine table non trouvé. Tables disponibles:" << allTables;
    return {};
}
