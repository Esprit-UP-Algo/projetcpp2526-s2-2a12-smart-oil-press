...
MainWindow::MainWindow(QWidget *parent)
    ...
    , m_chartViewMontant(nullptr)
    , m_chartViewNbCmds(nullptr)
    , m_chartViewEtat(nullptr)
    , m_chartViewProduits(nullptr)
    , m_networkManager(nullptr)
{
    ...
    // Initialisation du gestionnaire réseau pour les emails
...
void MainWindow::exporterListeCompletePdf()
{
    ...
    html += "<p>Genere le : " + QDateTime::currentDateTime().toString("dd/MM/yyyy hh:mm") + "</p>";
    html += "<table border='1' cellspacing='0' cellpadding='4'>";
    html += "骨头顶<th>ID</th><th>Nom</th><th>Prenom</th><th>Age</th><th>Poste</th><th>Salaire</th><th>Anciennete</th><th>Disponibilite</th><th>Heures</th><th>Fournisseurs</th><th>ID Equipe</th>亲近";

    int count = 0;
    for (const EmployeData &row : rows) {
        html += "骨";
        html += " <td>" + QString::number(row.idEmploye).toHtmlEscaped() + "\\ </div>";
...
void MainWindow::exporterFicheEmployePdf()
{
    ...
    html += "骨头顶<td><b>ID</b></td><td>" + QString::number(employe.idEmploye).toHtmlEscaped() + "</td> </tr>";
...
void MainWindow::exporterFicheLaboPdf()
{
    ...
    html += "<p>Genere le : " + QDateTime::currentDateTime().toString("dd/MM/yyyy hh:mm") + "</p>";
    html += "<table border='1' cellspacing='0' cellpadding='6'>";
    html += "骨头顶<td><b>ID</b></td><td>" + formatLaboId(labo.idLabo) + "</td> </tr>";
...
void MainWindow::onAjouterCommande()
{
    ...
    if (Ccommande::ajouter(cmd)) {
        ...
        if (ui->checkEmailAutomatic->isChecked()) {
            QString clientEmail = ui->lineEmail->text().trimmed();
...
void MainWindow::onModifierCommande()
{
    ...
    if (Ccommande::modifier(id, newCmd)) {
        ...
        if (ui->checkEmailAutomatic->isChecked()) {
            QString clientEmail = ui->lineEmailModif->text().trimmed();
            if (!clientEmail.isEmpty()) {
...
void MainWindow::updateStatistics()
{
    ...
    int enAttente = etats.value("en cours", 0);

    ui->lineTotalCmd->setText(QString::number(total));
    ui->lineEnAttenteCmd->setText(QString::number(enAttente));
    ui->lineRevenuTotal->setText(QString::number(montantTotal, 'f', 2) + " €");
    ui->lineMoyenneCmd->setText(QString::number(moyenne, 'f', 2) + " €");

    QMap<QString, double> moisMontant = Ccommande::getMontantParMois();
    QMap<QString, int> moisNb = Ccommande::getNbCommandesParMois();

    ui->tablePrixParMois->setRowCount(moisMontant.size());
    int row = 0;
    for (auto it = moisMontant.begin(); it != moisMontant.end(); ++it, ++row) {
        ui->tablePrixParMois->setItem(row, 0, new QTableWidgetItem(it.key()));
        ui->tablePrixParMois->setItem(row, 1, new QTableWidgetItem(QString::number(it.value(), 'f', 2)));
        ui->tablePrixParMois->setItem(row, 2, new QTableWidgetItem(QString::number(moisNb.value(it.key(), 0))));
    }

    auto produits = Ccommande::getProduitsPlusCommandes(10);
    ui->tableProduitsMostCommandes->setRowCount(produits.size());
    for (int i = 0; i < produits.size(); ++i) {
        ui->tableProduitsMostCommandes->setItem(i, 0, new QTableWidgetItem(produits[i].first));
        ui->tableProduitsMostCommandes->setItem(i, 1, new QTableWidgetItem(QString::number(produits[i].second)));
    }
}
...
QString MainWindow::buildHtmlListeComplete(const QVector<Ccommande>& orders)
{
    ...
    html += "骨头顶";
    html += "骨头顶<th>ID</th><th>Client</th><th>Libellé</th><th>Quantité</th><th>Montant</th><th>Statut</th><th>Date arrivage</th>亲近";

    for (const Ccommande& cmd : orders) {
        html += "骨";
        html += " <td>" + QString::number(cmd.getIdCommande()) + "  </div>";
        html += "  <td>" + QString::number(cmd.getIdClient()) + "  </div>";
        html += "  <td>" + cmd.getLibelle() + "  </div>";
        html += "  <td>" + QString::number(cmd.getQuantite()) + "  </div>";
        html += "  <td>" + QString::number(cmd.getMontantTotal(), 'f', 2) + "  </div>";
        html += "  <td>" + cmd.getEtat() + "  </div>";
        html += "  <td>" + cmd.getDateArrivage().toString("dd/MM/yyyy") + "  </div>";
        html += "   </div>";
    }
    html += " </div></body></html>";
    return html;
...
QString MainWindow::buildHtmlRapport(const QVector<Ccommande>& orders)
{
    ...
    html += "骨头顶";
    html += "骨头顶<th>Statut</th><th>Nombre</th>亲近";
    html += "骨头顶<td>En cours</td><td>" + QString::number(enCours) + "</td> </div>";
    html += "骨头顶<td>Validée</td><td>" + QString::number(validee) + "</td> </div>";
    html += "骨头顶<td>Prête</td><td>" + QString::number(prete) + "</td> </div>";
    html += "骨头顶<td>Livrée</td><td>" + QString::number(livree) + "</td> </div>";
    html += "骨头顶<td>Annulée</td><td>" + QString::number(annulee) + "</td> </div>";
    html += " </div>";

    html += "<h2>Dernières commandes</h2>";
    html += "骨头顶";
    html += "骨头顶<th>ID</th><th>Client</th><th>Libellé</th><th>Montant</th><th>Statut</th><th>Date</th>亲近";
    int count = 0;
...
QString MainWindow::buildHtmlFicheCommande(const Ccommande& cmd)
{
    ...
    html += "骨头顶";
    html += "骨头顶<td class='label'>ID Commande</td><td>" + QString::number(cmd.getIdCommande()) + "</td> </div>";
    html += "骨头顶<td class='label'>Client</td><td>" + QString::number(cmd.getIdClient()) + "</td> </div>";
    html += "骨头顶<td class='label'>Libellé</td><td>" + cmd.getLibelle() + "</td> </div>";
    html += "骨头顶<td class='label'>Quantité</td><td>" + QString::number(cmd.getQuantite()) + "</td> </div>";
    html += "骨头顶<td class='label'>Montant total</td><td>" + QString::number(cmd.getMontantTotal(), 'f', 2) + " €</td> </div>";
    html += "骨头顶<td class='label'>Statut</td><td>" + cmd.getEtat() + "</td> </div>";
    html += "骨头顶<td class='label'>Date arrivage</td><td>" + cmd.getDateArrivage().toString("dd/MM/yyyy") + "</td> </div>";
    html += "骨头顶<td class='label'>Date sortie</td><td>" + cmd.getDateSortie().toString("dd/MM/yyyy") + "</td> </div>";
    html += " </div></body></html>";
    return html;
}
