...
static bool matchesTelephonePattern(const QString &fieldName)
{
    const QString normalized = fieldName.trimmed().toLower();
    const QString target = "telephone";
    int tIndex = 0;

    for (int i = 0; i < normalized.size() && tIndex < target.size(); ++i) {
        const QChar c = normalized.at(i);
        if (c.isLetter()) {
            if (c == target.at(tIndex)) {
                ++tIndex;
            } else {
                return false;
            }
        } else {
            ++tIndex; // allow a placeholder character to match one target letter
        }
    }
    return tIndex == target.size();
}

static bool isTelephoneFieldName(const QString &fieldName)
{
    const QString normalized = normalizeToken(fieldName);
    if (normalized == "telephone"
        || normalized == "tel"
        || normalized == "numerotelephone"
        || normalized == "numtel") {
        return true;
    }
    return normalized.contains("telephone")
        || normalized.contains("tel")
        || matchesTelephonePattern(fieldName);
}

static QString resolveClientTelephoneColumn()
{
    static QString cachedColumn;
    ...
    QSqlQuery q;
    if (q.exec("SELECT * FROM CLIENT WHERE 1=0")) {
        QSqlRecord record = q.record();
        for (int i = 0; i < record.count(); ++i) {
            const QString fieldName = record.fieldName(i);
            if (isTelephoneFieldName(fieldName)) {
                cachedColumn = fieldName;
                qDebug() << "[Client] resolved telephone column from record:" << cachedColumn;
                return cachedColumn;
            }
        }
    }
    ...
    QSqlQuery q2;
    if (q2.exec("SELECT COLUMN_NAME FROM USER_TAB_COLUMNS WHERE TABLE_NAME = 'CLIENT' "
                "AND REGEXP_LIKE(REGEXP_REPLACE(COLUMN_NAME, '[^A-Za-z0-9]', ''), '^T.*L.*P.*H.*O.*N.*E$', 'i')")) {
        while (q2.next()) {
            const QString columnName = q2.value(0).toString();
            cachedColumn = columnName;
            qDebug() << "[Client] resolved telephone column from USER_TAB_COLUMNS with regex:" << cachedColumn;
            return cachedColumn;
        }
    }
    if (q2.lastError().isValid()) {
        qDebug() << "[Client] USER_TAB_COLUMNS lookup error:" << q2.lastError().text();
    }

    cachedColumn = "TELEPHONE";
    qDebug() << "[Client] failed to detect telephone column, fallback to" << cachedColumn;
    return cachedColumn;
}
} // namespace

// Oracle DB table: CLIENT
// Columns by position: 0=ID_CLIENT, 1=NOM_CLIENT, 2="TÉLÉPHONE", 3=EMAIL,
//                      4=ADRESSE, 5=TYPE_CLIENT, 6=STATUT, 7=ID_EMPLOYE
// The TELEPHONE column is stored with an accented name in Oracle.
// Use a quoted identifier for the persisted column name to avoid invalid identifier errors.
...
bool Client::ajouter(QString *errorMessage)
{
    ...
    const QString telephoneColumn = resolveClientTelephoneColumn();
    const QString insertSql = QString(
        "INSERT INTO CLIENT (ID_CLIENT, NOM_CLIENT, %1, EMAIL, TYPE_CLIENT, STATUT) "
        "VALUES (:id, :nom, :tel, :email, :type, :statut)")
        .arg(quoteIdentifier(telephoneColumn));
...
bool Client::modifier(QString *errorMessage)
{
    ...
    const QString updateSql = QString(
        "UPDATE CLIENT SET "
        "NOM_CLIENT = :nom, %1 = :tel, EMAIL = :email, TYPE_CLIENT = :type, STATUT = :statut "
        "WHERE ID_CLIENT = :id")
        .arg(quoteIdentifier(telephoneColumn));
