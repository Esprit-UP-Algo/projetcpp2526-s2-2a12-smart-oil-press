...
#include "utils/qrcode/qrcodegen.hpp"
...
QString resolveMachineQrCodesPath()
{
    return QDir(QStringLiteral(APP_PROJECT_SOURCE_DIR)).absoluteFilePath("qr_codes");
}
...
PlanningRateData parsePlanningRateData(const QString &rateRaw)
{
    ...
    if (!normalized.contains("||")) {
        ...
        if (ok) {
            data.computedRate = QString::number(directRate, 'f', 2);
        }
        return data;
    }
    ...
    for (const QString &counter : counters) {
        ...
        if (token.startsWith("S:")) {
            ...
            const QString rateText = token.mid(2).trimmed();
            data.computedRate = rateText.isEmpty() ? "0" : rateText;
        }
    }
...
QString serializePlanningRateData(const PlanningRateData &data)
{
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        tasks.append(QString("%1,%2,%3,%4").arg(slot.code, slot.description, slot.startDate, slot.endDate));
    }
...
{
    const QString value = uiSpecialty.trimmed().toLower();
    if (value == "maintenance" || value == "maintenace") {
        return "maintenance";
    }
    if (value == "production") {
        return "production";
    }
    if (value == "back-office" || value == "backoffice") {
        return "backoffice";
    }
    if (value == "mécanique" || value == "mecanique") {
        return "mecanique";
    }
    return value;
}
    ...
    , ui(new Ui::MainWindow)
{
    ...
    // NOTE: Fingerprint Arduino auto-connect removed - now manual via button to avoid port conflicts
    // User must click Connect button for fingerprint Arduino
    
    setupEnvironmentArduino();

    // m_fingerprintSerial will be set when user clicks Connect button
    m_fingerprintSerial = nullptr;
    ...
    m_fingerprintTimeoutTimer = new QTimer(this);
    m_fingerprintTimeoutTimer->setSingleShot(true);
    connect(m_fingerprintTimeoutTimer, &QTimer::timeout, this, [this]() {
        failFingerprintOperation("Temps d'attente depasse. Relancez le scan.");
    });
    ...
    auto setEquipePageFromTab = [this](int index) {
        ...
        case 6: // Planification
            ...
            showTasksForDate(ui->calendarWidget ? ui->calendarWidget->selectedDate() : QDate::currentDate());
            break;
        default:
            break;
        }
    ...
    auto *btnFingerprintAuth = new QPushButton("Authentifier avec empreinte", ui->loginFormContainer);
    btnFingerprintAuth->setObjectName("btnFingerprintAuth");
    btnFingerprintAuth->setCursor(Qt::PointingHandCursor);
    btnFingerprintAuth->setMinimumHeight(42);
    btnFingerprintAuth->setStyleSheet(
        "QPushButton { background-color: #F8EDE3; color: #798777; border: 1px solid #BDD2B6; "
        "border-radius: 6px; padding: 10px 18px; font-weight: bold; }"
        "QPushButton:hover { background-color: #FFF6EF; border-color: #A2B29F; }");
    if (auto *loginLayout = qobject_cast<QVBoxLayout *>(ui->loginFormContainer->layout())) {
        const int forgotIndex = loginLayout->indexOf(ui->btnNavigateForgot);
        if (forgotIndex != -1) {
            loginLayout->insertWidget(forgotIndex + 1, btnFingerprintAuth);
        } else {
            loginLayout->addWidget(btnFingerprintAuth);
        }
    }
    connect(btnFingerprintAuth, &QPushButton::clicked, this, &MainWindow::openFingerprintAuthDialog);
    setupFingerprintEnrollTab();
    ...
    connect(ui->btnSeConnecter, &QPushButton::clicked, this, [this]() {
        ...
        completeEmployeAuthentication(username, role);
    });
    ...
    resetForgotFlowState(true);
    setEmployeAuthenticated(false);
    
    // Setup machine connections
    setupMachineConnections();

    // === PARKING RFID MODULE — embedded as a tab inside WidgetClient ===
    m_parkingWidget = new ParkingWidget(this);
    m_parkingWidget->setArduino(&A);

    // Find the Parking tab placeholder in tabWidgetClient and replace it
    if (ui->tabWidgetClient && ui->tabParkingClient) {
        // Remove the placeholder label from the tab
        QLayout *tabLayout = ui->tabParkingClient->layout();
        if (tabLayout) {
            QLayoutItem *item;
            ...
            tabLayout->addWidget(m_parkingWidget);
        }
    }
...
// === SLOTS GENERAUX (navigation, menu, retour) ===
void MainWindow::setEmployeAuthenticated(bool authenticated)
{
    if (!ui->tabWidget) {
        return;
    }
    ...
    if (authenticated) {
        ...
        setTabVisible(m_fingerprintEnrollTab, true);
        ...
        setTabVisible(m_fingerprintEnrollTab, false);
...
void MainWindow::completeEmployeAuthentication(const QString &identity, const QString &role)
{
    m_currentRole = role;
    ...
    if (ui->line_user_conn) {
        ui->line_user_conn->setText(identity);
    }
    if (ui->line_pass_conn) {
        ui->line_pass_conn->clear();
    }

    setEmployeAuthenticated(true);
    if (ui->stackedMain && ui->pageAccueil) {
...
void MainWindow::update_label()
{
    ...
    m_fingerprintBuffer += data;
    handleFingerprintSerialReadyRead();
}

bool MainWindow::ensureFingerprintSerialConnected(QString *errorMessage)
{
    // Mutual exclusion: Disconnect Environment Arduino if connected
    if (m_envArduino.isConnected()) {
        m_envArduino.disconnect();
        updateEnvArduinoStatus();
        qDebug() << "[Fingerprint] Disconnected Environment Arduino for mutual exclusion";
        ui->statusbar1->showMessage("Capteur environnement deconnecte pour utiliser l'empreinte", 5000);
    }

    const int ret = A.connect_arduino();
    m_fingerprintSerial = A.getserial();

    if (ret == 0 && m_fingerprintSerial && m_fingerprintSerial->isOpen()) {
        return true;
    }

    if (errorMessage) {
        if (ret == 1) {
            *errorMessage = "Arduino detecte mais connexion serie impossible.";
        } else {
            *errorMessage = "Impossible de se connecter a l'Arduino d'empreinte. Verifiez le port serie.";
        }
    }
    return false;
}

bool MainWindow::sendFingerprintCommand(const QString &command, QString *errorMessage)
{
    if (!ensureFingerprintSerialConnected(errorMessage)) {
        return false;
    }

    const QByteArray payload = command.endsWith('\n')
        ? command.toUtf8()
        : (command + "\n").toUtf8();

    m_fingerprintBuffer.clear();
    A.write_to_arduino(payload);
    if (!m_fingerprintSerial || !m_fingerprintSerial->waitForBytesWritten(1000)) {
        if (errorMessage) {
            *errorMessage = "Envoi impossible vers l'Arduino.";
        }
        return false;
    }
    return true;
...
QString MainWindow::buildArduinoDisplayMessage(const QString &prefix, const QString &identity) const
{
    QString message = (prefix + " " + identity).simplified();
    message.replace('\n', ' ');
    message.replace('\r', ' ');
    return message.left(32);
}

void MainWindow::sendArduinoDisplayCommand(const QString &commandPrefix, const QString &message)
{
    QString errorMessage;
    if (!sendFingerprintCommand(commandPrefix + message, &errorMessage)) {
        qDebug() << "[Empreinte] Envoi LCD Arduino impossible:" << errorMessage;
    }
}

void MainWindow::clearFingerprintOperationState()
{
    m_fingerprintBuffer.clear();
    m_fingerprintOperation.clear();
    m_pendingFingerprintEmployeId = -1;
    m_pendingFingerprintId = -1;
    if (m_fingerprintTimeoutTimer) {
        m_fingerprintTimeoutTimer->stop();
    }
}

void MainWindow::failFingerprintOperation(const QString &message)
{
    if (m_fingerprintAuthStatusLabel && m_fingerprintAuthDialog) {
        m_fingerprintAuthStatusLabel->setText(message);
    }
    if (m_fingerprintEnrollStatusLabel) {
        m_fingerprintEnrollStatusLabel->setText(message);
    }
    if (m_fingerprintEnrollStartButton) {
        m_fingerprintEnrollStartButton->setEnabled(true);
    }
    clearFingerprintOperationState();
}

void MainWindow::finishFingerprintAuthentication(int fingerprintId)
{
    QString identity;
    QString role;
    QString errorMessage;
    if (!EmployeRepository::authenticateByFingerprintId(
            fingerprintId, &identity, &role, nullptr, &errorMessage)) {
        failFingerprintOperation(errorMessage);
        QMessageBox::warning(this, "Empreinte", errorMessage);
        return;
    }

    if (m_fingerprintAuthStatusLabel) {
        m_fingerprintAuthStatusLabel->setText("Empreinte reconnue. Connexion de " + identity + "...");
    }

    sendArduinoDisplayCommand(
        "SUCCESS:",
        buildArduinoDisplayMessage("Bienvenue", identity));

    clearFingerprintOperationState();
    if (m_fingerprintAuthDialog) {
        m_fingerprintAuthDialog->accept();
        m_fingerprintAuthDialog = nullptr;
        m_fingerprintAuthStatusLabel = nullptr;
    }

    completeEmployeAuthentication(identity, role);
}

void MainWindow::finishFingerprintEnrollment(int fingerprintId)
{
    QString identity;
    QString errorMessage;
    if (!EmployeRepository::assignFingerprintId(
            m_pendingFingerprintEmployeId, fingerprintId, &identity, &errorMessage)) {
        failFingerprintOperation(errorMessage);
        QMessageBox::warning(this, "Empreinte", errorMessage);
        return;
    }
    ...
    if (m_fingerprintEnrollStatusLabel) {
        m_fingerprintEnrollStatusLabel->setText(
            QString("Empreinte %1 associee a %2.").arg(fingerprintId).arg(identity));
    }
    if (m_fingerprintEnrollStartButton) {
        m_fingerprintEnrollStartButton->setEnabled(true);
    }
    if (m_fingerprintEnrollEmployeIdEdit) {
        m_fingerprintEnrollEmployeIdEdit->clear();
    }
    if (m_fingerprintEnrollIdentityEdit) {
        m_fingerprintEnrollIdentityEdit->clear();
    }

    sendArduinoDisplayCommand(
        "ajout:",
        buildArduinoDisplayMessage("Empreinte", identity + " enregistree"));

    clearFingerprintOperationState();
    QMessageBox::information(
        this,
        "Empreinte",
        QString("Empreinte enregistree avec succes pour %1.").arg(identity));
}

void MainWindow::handleFingerprintSerialReadyRead()
{
    const QString serialText = QString::fromLatin1(m_fingerprintBuffer);

    if (serialText.contains("Did not find a match", Qt::CaseInsensitive)
        || serialText.contains("USER NOT FOUND", Qt::CaseInsensitive)) {
        failFingerprintOperation("Aucune empreinte correspondante n'a ete trouvee.");
        return;
    }

    if (m_fingerprintOperation == "auth") {
        static const QRegularExpression authRegex("Found ID\\s*#\\s*(\\d+)");
        const QRegularExpressionMatch match = authRegex.match(serialText);
        if (match.hasMatch()) {
            finishFingerprintAuthentication(match.captured(1).toInt());
        }
        return;
    }

    if (m_fingerprintOperation == "enroll") {
        static const QRegularExpression enrollRegex("Stored ID:\\s*(\\d+)");
        const QRegularExpressionMatch match = enrollRegex.match(serialText);
        if (match.hasMatch()) {
            finishFingerprintEnrollment(match.captured(1).toInt());
        }
    }
}

void MainWindow::openFingerprintAuthDialog()
{
    QString errorMessage;
    if (!ensureFingerprintSerialConnected(&errorMessage)) {
        QMessageBox::warning(this, "Empreinte", errorMessage);
    ...
    if (m_fingerprintAuthDialog) {
        m_fingerprintAuthDialog->raise();
        m_fingerprintAuthDialog->activateWindow();
        return;
    }

    auto *dialog = new QDialog(this);
    dialog->setAttribute(Qt::WA_DeleteOnClose);
    dialog->setWindowTitle("Authentification par empreinte");
    dialog->setModal(true);
    dialog->resize(420, 180);

    auto *layout = new QVBoxLayout(dialog);
    auto *title = new QLabel("Scan d'empreinte en cours", dialog);
    title->setStyleSheet("font-size: 16px; font-weight: bold; color: #798777;");
    auto *message = new QLabel(
        "Posez votre doigt sur le capteur. L'application attend la reponse de l'Arduino.",
        dialog);
    message->setWordWrap(true);
    auto *closeButton = new QPushButton("Annuler", dialog);
    ...
    m_fingerprintAuthDialog = dialog;
    m_fingerprintAuthStatusLabel = message;
    ...
    m_fingerprintOperation = "auth";
    if (!sendFingerprintCommand("SEARCH", &errorMessage)) {
        dialog->reject();
        QMessageBox::warning(this, "Empreinte", errorMessage);
        return;
    }
    ...
    if (m_fingerprintTimeoutTimer) {
        m_fingerprintTimeoutTimer->start(15000);
    }
}

void MainWindow::updateFingerprintEnrollIdentity()
{
    if (!m_fingerprintEnrollEmployeIdEdit || !m_fingerprintEnrollIdentityEdit) {
        return;
    }

    const QString employeIdText = m_fingerprintEnrollEmployeIdEdit->text().trimmed();
    if (employeIdText.isEmpty()) {
        m_fingerprintEnrollIdentityEdit->clear();
        if (m_fingerprintEnrollStatusLabel) {
            m_fingerprintEnrollStatusLabel->setText("Saisissez un ID employe pour charger son nom.");
        }
        return;
    }
    ...
    QString identity;
    QString errorMessage;
    if (!EmployeRepository::getEmployeIdentityById(employeIdText.toInt(), &identity, &errorMessage)) {
        m_fingerprintEnrollIdentityEdit->clear();
        if (m_fingerprintEnrollStatusLabel) {
            m_fingerprintEnrollStatusLabel->setText(errorMessage);
        }
        return;
    }

    m_fingerprintEnrollIdentityEdit->setText(identity);
    if (m_fingerprintEnrollStatusLabel) {
        m_fingerprintEnrollStatusLabel->setText(
            QString("Employe charge. L'empreinte sera enregistree avec l'ID %1.")
                .arg(employeIdText));
    }
}

void MainWindow::startFingerprintEnrollment()
{
    if (!m_fingerprintEnrollEmployeIdEdit || !m_fingerprintEnrollIdentityEdit) {
        return;
    }

    const int employeId = m_fingerprintEnrollEmployeIdEdit->text().trimmed().toInt();
    if (employeId <= 0) {
        QMessageBox::warning(this, "Empreinte", "Veuillez saisir un ID employe valide.");
        return;
    }
    if (m_fingerprintEnrollIdentityEdit->text().trimmed().isEmpty()) {
        QMessageBox::warning(this, "Empreinte", "Chargement du nom employe impossible.");
        return;
    }
    ...
    m_pendingFingerprintEmployeId = employeId;
    m_pendingFingerprintId = employeId;
    m_fingerprintOperation = "enroll";
    m_fingerprintBuffer.clear();
    if (m_fingerprintEnrollStartButton) {
        m_fingerprintEnrollStartButton->setEnabled(false);
    }
    if (m_fingerprintEnrollStatusLabel) {
        m_fingerprintEnrollStatusLabel->setText(
            QString("Enrolement en cours pour %1. Le capteur utilisera l'ID %2.")
                .arg(m_fingerprintEnrollIdentityEdit->text().trimmed())
                .arg(employeId));
    }

    if (!sendFingerprintCommand(QString("ENROLL:%1").arg(employeId), &errorMessage)) {
        if (m_fingerprintEnrollStartButton) {
            m_fingerprintEnrollStartButton->setEnabled(true);
        }
        clearFingerprintOperationState();
    ...
    if (m_fingerprintTimeoutTimer) {
        m_fingerprintTimeoutTimer->start(25000);
    }
}

void MainWindow::setupFingerprintEnrollTab()
{
    if (!ui->tabWidget || m_fingerprintEnrollTab) {
        return;
    }

    auto *tab = new QWidget(ui->tabWidget);
    tab->setObjectName("tabEmpreinte");

    auto *mainLayout = new QVBoxLayout(tab);
    auto *introBox = new QGroupBox("Enregistrement d'empreinte", tab);
    auto *introLayout = new QVBoxLayout(introBox);
    auto *introLabel = new QLabel(
        "Saisissez uniquement l'ID de l'employe. Son username sera affiche automatiquement "
        "sous la forme Nom Prenom, puis le capteur enregistrera l'empreinte avec le meme ID.",
        introBox);
    introLabel->setWordWrap(true);
    introLayout->addWidget(introLabel);

    auto *formBox = new QGroupBox("Parametres", tab);
    auto *formLayout = new QFormLayout(formBox);
    auto *employeIdEdit = new QLineEdit(formBox);
    auto *identityEdit = new QLineEdit(formBox);
    auto *statusLabel = new QLabel("Saisissez un ID employe pour afficher automatiquement le username.", tab);
    auto *startButton = new QPushButton("Enregistrer l'empreinte", tab);

    employeIdEdit->setValidator(new QIntValidator(1, 999999, tab));
    employeIdEdit->setPlaceholderText("ID employe");
    identityEdit->setReadOnly(true);
    identityEdit->setPlaceholderText("Nom Prenom");
    statusLabel->setWordWrap(true);
    startButton->setMinimumHeight(44);
    startButton->setCursor(Qt::PointingHandCursor);

    formLayout->addRow("ID employe", employeIdEdit);
    formLayout->addRow("Username", identityEdit);
    ...
    const int exportIndex = ui->tabWidget->indexOf(ui->tabExportPdf);
    if (exportIndex != -1) {
        ui->tabWidget->insertTab(exportIndex + 1, tab, "Empreinte");
    } else {
        ui->tabWidget->addTab(tab, "Empreinte");
    }

    m_fingerprintEnrollTab = tab;
    m_fingerprintEnrollEmployeIdEdit = employeIdEdit;
    m_fingerprintEnrollIdentityEdit = identityEdit;
    m_fingerprintEnrollStatusLabel = statusLabel;
    m_fingerprintEnrollStartButton = startButton;

    connect(employeIdEdit, &QLineEdit::textChanged, this, [this](const QString &) {
        updateFingerprintEnrollIdentity();
    });
    connect(startButton, &QPushButton::clicked, this, &MainWindow::startFingerprintEnrollment);
}
...
void MainWindow::setupLaboConnections()
{
    ...
    if (!m_tunisiaMapWidget) {
        try {
            ...
            // Connect governorate click signal
            connect(m_tunisiaMapWidget, &TunisiaMapWidget::governorateClicked, 
                    this, &MainWindow::onGovernorateClicked);
...
void MainWindow::onTableRowClicked(int row, int column)
{
    ...
    if (ui->table_employes->item(row, 1)) {
        nom = ui->table_employes->item(row, 1)->text().trimmed();
        ui->lineEdit_nom_mod->setText(nom);
    }
    if (ui->table_employes->item(row, 2)) {
        prenom = ui->table_employes->item(row, 2)->text().trimmed();
        ui->lineEdit_prenom_mod->setText(prenom);
    }
    ...
    if (m_fingerprintEnrollEmployeIdEdit) {
        m_fingerprintEnrollEmployeIdEdit->setText(id);
    }
    if (m_fingerprintEnrollIdentityEdit) {
        m_fingerprintEnrollIdentityEdit->setText((nom + " " + prenom).simplified());
    }
    if (m_fingerprintEnrollStatusLabel) {
        m_fingerprintEnrollStatusLabel->setText(
            QString("Employe selectionne depuis la liste. L'empreinte sera enregistree avec l'ID %1.")
                .arg(id));
    }
}
...
void MainWindow::setupEquipeConnections()
{
    ...
    connect(ui->lineEdit_teamDetailsId, &QLineEdit::returnPressed,
        ...
        connect(ui->btnRechercherRecommandationEquipe, &QPushButton::clicked,
            this, &MainWindow::on_btnRunSuggestionSearch_clicked);
        connect(ui->pushButton_exportGlobalStatsPdf, &QPushButton::clicked,
            this, &MainWindow::on_pushButton_exportGlobalStatsPdf_clicked);
        connect(ui->pushButton_exportTeamDetailsPdf, &QPushButton::clicked,
            this, &MainWindow::on_pushButton_exportTeamDetailsPdf_clicked);
        connect(ui->pushButton_validateTeam, &QPushButton::clicked,
            this, &MainWindow::on_pushButton_validateTeam_clicked);
        connect(ui->lineEdit_teamId, &QLineEdit::returnPressed,
            this, &MainWindow::on_pushButton_validateTeam_clicked);
        connect(ui->pushButton_addTask, &QPushButton::clicked,
            this, &MainWindow::on_pushButton_addTask_clicked);
        connect(ui->pushButton_deleteTask, &QPushButton::clicked,
            this, &MainWindow::on_pushButton_deleteTask_clicked);
...
void MainWindow::on_pushButton_validateTeam_clicked()
{
    refreshEquipeTable();

    const QString teamIdText = ui->lineEdit_teamId ? ui->lineEdit_teamId->text().trimmed() : QString();
    if (teamIdText.isEmpty()) {
        if (ui->label_teamStatus) {
            ui->label_teamStatus->clear();
            ui->label_teamStatus->setStyleSheet("color: gray; font-style: italic;");
        }
        m_selectedTeamIdForPlanning = -1;
        m_selectedTeamIsActiveForPlanning = false;
        if (ui->calendarWidget) {
            showTasksForDate(ui->calendarWidget->selectedDate());
        }
        return;
    ...
    bool ok = false;
    const int teamId = teamIdText.toInt(&ok);
    if (!ok || teamId <= 0) {
        if (ui->label_teamStatus) {
    ...
    const int teamIndex = findEquipeIndexById(teamId);
    if (teamIndex < 0 || teamIndex >= m_equipes.size()) {
        if (ui->label_teamStatus) {
            ui->label_teamStatus->setText("Équipe non trouvée. Vérifier le ID de l'équipe.");
            ui->label_teamStatus->setStyleSheet("color: red;");
        }
        m_selectedTeamIdForPlanning = -1;
        m_selectedTeamIsActiveForPlanning = false;
    ...
    m_selectedTeamIdForPlanning = teamId;
    EquipeData team = m_equipes.at(teamIndex);
    PlanningRateData planning = parsePlanningRateData(team.rate);

    if (applyOverdueFailures(planning, QDate::currentDate())) {
        team.rate = serializePlanningRateData(planning);
        int rowsAffected = 0;
        QString dbError;
        if (!EquipeRepository::modifierEquipe(team, &rowsAffected, &dbError)) {
        ...
        refreshEquipeTable();
        const int refreshedIndex = findEquipeIndexById(teamId);
        if (refreshedIndex < 0 || refreshedIndex >= m_equipes.size()) {
            if (ui->label_teamStatus) {
                ui->label_teamStatus->setText("Équipe non trouvée. Vérifier le ID de l'équipe.");
                ui->label_teamStatus->setStyleSheet("color: red;");
            }
        ...
        team = m_equipes.at(refreshedIndex);
        planning = parsePlanningRateData(team.rate);
    }

    m_selectedTeamIsActiveForPlanning = isTeamActive(team.etat);
    if (ui->label_teamStatus) {
        if (m_selectedTeamIsActiveForPlanning) {
            ui->label_teamStatus->setText("Équipe sélectionnée: " + team.nomEquipe);
            ui->label_teamStatus->setStyleSheet("color: green; font-weight: bold;");
        } else {
            ui->label_teamStatus->setText(
                QString("Équipe %1: désolé, cette équipe est inactive pour le moment.").arg(team.nomEquipe));
...
void MainWindow::showTasksForDate(const QDate &date)
{
    if (!ui->planningTableWidget) {
        return;
    }
    ...
    QVector<EquipeData> teamsToDisplay;
    QString selectedTeamName;
    if (m_selectedTeamIdForPlanning > 0) {
        const int selectedIndex = findEquipeIndexById(m_selectedTeamIdForPlanning);
        if (selectedIndex >= 0 && selectedIndex < m_equipes.size()) {
            teamsToDisplay.append(m_equipes[selectedIndex]);
            selectedTeamName = m_equipes[selectedIndex].nomEquipe;
        }
    }
    if (teamsToDisplay.isEmpty()) {
        teamsToDisplay = QVector<EquipeData>(m_equipes.cbegin(), m_equipes.cend());
    }
    ...
    for (const EquipeData &team : teamsToDisplay) {
        const PlanningRateData planning = parsePlanningRateData(team.rate);
        for (const PlannedTaskSlot &slot : planning.taskSlots) {
            if (isEmptyTaskSlot(slot)) {
                continue;
            }
            ...
            if (date >= startDate && date <= endDate) {
                ...
                taskItem->setData(Qt::UserRole + 1, team.idEquipe);
                ...
                const int teamId = team.idEquipe;
                ...
                ui->planningTableWidget->setItem(row, 1, new QTableWidgetItem(team.nomEquipe));
...
void MainWindow::completeTaskSlotForTeam(int teamId, const QString &slotCode)
{
    if (slotCode.trimmed().isEmpty() || teamId <= 0) {
        return;
    }

    int teamIndex = findEquipeIndexById(teamId);
    if (teamIndex < 0 || teamIndex >= m_equipes.size()) {
        refreshEquipeTable();
        teamIndex = findEquipeIndexById(teamId);
    }
    ...
    QString dbError;
    if (!EquipeRepository::modifierEquipe(team, &rowsAffected, &dbError)) {
        QMessageBox::critical(this, "Erreur", "Impossible de valider la tâche:\n" + dbError);
        return;
    }
    ...
    if (m_selectedTeamIdForPlanning == teamId && ui->label_teamStatus) {
        ui->label_teamStatus->setText("Équipe sélectionnée: " + team.nomEquipe);
        ui->label_teamStatus->setStyleSheet("color: green; font-weight: bold;");
    }
}

void MainWindow::deleteTaskSlotForTeam(int teamId, const QString &slotCode)
{
    if (slotCode.trimmed().isEmpty() || teamId <= 0) {
        return;
    }
    ...
    QString dbError;
    if (!EquipeRepository::modifierEquipe(team, &rowsAffected, &dbError)) {
        QMessageBox::critical(this, "Erreur", "Impossible de supprimer la tâche:\n" + dbError);
        return;
    }
...
void MainWindow::on_pushButton_addTask_clicked()
{
    ...
    if (!m_selectedTeamIsActiveForPlanning) {
        if (ui->label_teamStatus) {
            ui->label_teamStatus->setText("Désolé, cette équipe est inactive pour le moment. Ajout de tâche impossible.");
            ui->label_teamStatus->setStyleSheet("color: #b00020; font-weight: bold;");
        }
        return;
    }
    ...
    if (description.contains(',') || description.contains(';') || description.contains('|')) {
        QMessageBox::warning(this, "Validation", "La description ne doit pas contenir les caractères: , ; |");
        return;
    }
    ...
    int teamIndex = findEquipeIndexById(m_selectedTeamIdForPlanning);
    if (teamIndex < 0 || teamIndex >= m_equipes.size()) {
        refreshEquipeTable();
        teamIndex = findEquipeIndexById(m_selectedTeamIdForPlanning);
    }
    ...
    QString dbError;
    if (!EquipeRepository::modifierEquipe(team, &rowsAffected, &dbError)) {
        QMessageBox::critical(this, "Erreur", "Impossible d'enregistrer la tâche:\n" + dbError);
        return;
    }
...
void MainWindow::on_pushButton_deleteTask_clicked()
{
    ...
    const int row = ui->planningTableWidget ? ui->planningTableWidget->currentRow() : -1;
    if (row < 0) {
    ...
    if (teamIdFromRow != m_selectedTeamIdForPlanning) {
...
void MainWindow::on_btnRunSuggestionSearch_clicked()
{
    refreshEquipeTable();
    if (ui->suggestionResultsTable) {
    ...
    const QString requestedSpecialtyRaw = ui->comboBoxSpecialiteRecommandation
        ? ui->comboBoxSpecialiteRecommandation->currentText().trimmed()
        : QString();
    ...
    const int minMembers = ui->lineEditNbMembresMinRecommandation
        ? ui->lineEditNbMembresMinRecommandation->text().trimmed().toInt(&membersOk)
        : 0;
    ...
    const QString difficultyRaw = ui->comboBoxDifficulteRecommandation
        ? ui->comboBoxDifficulteRecommandation->currentText().trimmed().toLower()
        : QString();
    ...
    const bool urgentCase = ui->checkBoxUrgentRecommandation
        && ui->checkBoxUrgentRecommandation->isChecked()
        && !(ui->checkBoxEtatNormalRecommandation && ui->checkBoxEtatNormalRecommandation->isChecked());
    const bool requiresAvailability = ui->checkBoxRequiertDisponibilite && ui->checkBoxRequiertDisponibilite->isChecked();
    ...
    for (const EquipeData &team : m_equipes) {
        ...
        result.teamId = team.idEquipe;
        result.teamName = team.nomEquipe;

        EquipeData normalizedTeam = team;
        EquipeRateData rateData;
        QString rateError;
        if (!EquipeRepository::extraireRateData(normalizedTeam.rate, &rateData, &rateError)) {
            QMessageBox::warning(this, "Equipe", rateError);
            return;
        }

        result.rate = rateData.rateValue;

        const QString teamSpecialty = toDbSpecialty(team.specialite);
        const bool active = isTeamActive(team.etat);
        ...
        if (team.nbreMembres < minMembers) {
        ...
        if (difficultyRaw.contains("normal") && result.rate < 5.0) {
            result.classification = downgradeClass(result.classification);
        } else if (difficultyRaw.contains("difficile") && result.rate < 7.0) {
            result.classification = downgradeClass(result.classification);
        }

        const bool highWorkload = (result.ongoingTasks >= 3) || (result.ongoingTasks >= result.maxTasks);
        if (urgentCase && highWorkload) {
            result.classification = downgradeClass(result.classification);
        }
...
void MainWindow::on_pushButton_exportGlobalStatsPdf_clicked()
{
    refreshEquipeTable();
    updateEquipeOverallStats();
    ...
    QChartView *chartView = ui->overallSpecialtyChartHost ? ui->overallSpecialtyChartHost->findChild<QChartView*>() : nullptr;
    QPixmap chartPixmap;
    if (chartView) {
        chartView->repaint();
        QApplication::processEvents();
        chartPixmap = chartView->grab();
    } else if (ui->overallSpecialtyChartHost) {
    ...
    html += QString("<p><b>Date d'export:</b> %1</p>").arg(QDateTime::currentDateTime().toString("dd/MM/yyyy HH:mm"));
    html += "<hr/>";
    html += "<h2>Section 1 - Vue globale</h2>";
    html += QString("<p>%1<br/>%2</p>")
        .arg(ui->label_totalTeams ? ui->label_totalTeams->text().toHtmlEscaped() : QString(),
             ui->label_specialtyCount ? ui->label_specialtyCount->text().toHtmlEscaped() : QString());
    ...
    if (!chartPixmap.isNull()) {
        html += QString(
            ...
            "</div>").arg(chartResourceUrl.toString());
    } else {
    ...
    html += "<h2>Section 4 - Meilleures performances</h2>";
...
void MainWindow::on_pushButton_exportTeamDetailsPdf_clicked()
{
    refreshEquipeTable();
    updateEquipeDetailsStats();

    bool ok = false;
    const int teamId = ui->lineEdit_teamDetailsId ? ui->lineEdit_teamDetailsId->text().trimmed().toInt(&ok) : 0;
    if (!ok || teamId <= 0) {
        QMessageBox::warning(this, "Export PDF", "Veuillez saisir un ID équipe valide pour exporter les détails.");
        return;
    }

    const int index = findEquipeIndexById(teamId);
    if (index < 0) {
    ...
    QString serviceError;
    if (!EquipeRepository::calculerDetails(team, &details, &serviceError)) {
        QMessageBox::warning(this, "Export PDF", serviceError);
        return;
    }
    ...
    html += QString("<p><b>Nom:</b> %1<br/><b>ID:</b> %2</p>")
        .arg(team.nomEquipe.toHtmlEscaped())
        .arg(team.idEquipe);
    ...
    html += QString("<tr><td style='border:1px solid #777; padding:6px;'>Tâches complétées</td><td style='border:1px solid #777; padding:6px;'>%1</td></tr>")
        .arg(QString::number(details.successCount).toHtmlEscaped());
    html += QString("<tr><td style='border:1px solid #777; padding:6px;'>Tâches échouées</td><td style='border:1px solid #777; padding:6px;'>%1</td></tr>")
        .arg(QString::number(details.failureCount).toHtmlEscaped());
    html += QString("<tr><td style='border:1px solid #777; padding:6px;'>Rate (/10)</td><td style='border:1px solid #777; padding:6px;'>%1</td></tr>")
        .arg(QString::number(details.rateValue, 'f', 2).toHtmlEscaped());
    html += "</table>";

    html += QString("<h3>Feedback</h3><p>%1</p>").arg(details.feedback.toHtmlEscaped());
...
void MainWindow::afficherLabos()
{
    ...
    // Update map widget with lab data
    if (m_tunisiaMapWidget) {
        m_tunisiaMapWidget->setLabosData(labos);
    }
...
void MainWindow::onGovernorateClicked(const QString &governorate)
{
    // Get all labs
    QList<LaboData> labos;
    QString errorMessage;
    ...
    QString region;
    for (const auto &gov : TunisiaMapData::getGovernoratesData()) {
    ...
    // Get labs in this governorate (improved matching)
    QList<LaboData> labosInGov;
    for (const auto &labo : labos) {
        // Extract governorate from address with improved matching
        QString laboGov = labo.adresse.trimmed();
        
        // Try exact match first
        if (laboGov.compare(governorate, Qt::CaseInsensitive) == 0) {
            labosInGov.append(labo);
            continue;
        }
        
        // Try to match where address contains the governorate name
        if (laboGov.contains(governorate, Qt::CaseInsensitive)) {
            labosInGov.append(labo);
            continue;
        }
        
        // Try to match by first part (before dash or comma)
        QStringList parts = laboGov.split(QRegularExpression("[\\-,;]"), Qt::SkipEmptyParts);
        if (!parts.isEmpty()) {
            QString firstPart = parts[0].trimmed();
            if (firstPart.compare(governorate, Qt::CaseInsensitive) == 0) {
                labosInGov.append(labo);
                continue;
            }
        }
    ...
    QString infoText = QString("<b>📍 Governorate: %1</b><br>").arg(governorate);
    infoText += QString("<b>🗺️ Region: %1</b><br><br>").arg(region);
    infoText += QString("<b>🔬 Laboratories in this governorate (%1):</b><br>").arg(labosInGov.size());
    
    if (labosInGov.isEmpty()) {
        infoText += "No laboratories found in this governorate.";
    } else {
        infoText += "<ul style='margin: 10px 0;'>";
        for (const auto &labo : labosInGov) {
            infoText += QString("<li><b>%1</b></li>").arg(labo.nomLabo);
            infoText += QString("<li style='margin-left: 20px; color: #666;'>📍 %1</li>").arg(labo.adresse);
            infoText += QString("<li style='margin-left: 20px; color: #666;'>✓ Conformity: %1</li>").arg(labo.conformite);
        }
        infoText += "</ul>";
    }
    ...
    msgBox.setWindowTitle(QString("Region Information - %1").arg(region));
...
void MainWindow::applySortingAndDisplay(const QVector<Ccommande>& source)
{
    ...
    ui->tableResultatsRecherche->setColumnCount(10);
    ui->tableResultatsRecherche->setHorizontalHeaderLabels({"ID", "Client", "Produit", "Montant", "Date", "Statut", "Température", "Humidité", "Alerte", "Conditions"});
    ...
    for (int i = 0; i < sorted.size(); ++i) {
        ...
        ui->tableResultatsRecherche->setItem(i, 6, new QTableWidgetItem(QString::number(cmd.getTemperature(), 'f', 2)));
        ui->tableResultatsRecherche->setItem(i, 7, new QTableWidgetItem(QString::number(cmd.getHumidite(), 'f', 2)));
        ui->tableResultatsRecherche->setItem(i, 8, new QTableWidgetItem(cmd.getAlerte()));
        ui->tableResultatsRecherche->setItem(i, 9, new QTableWidgetItem(cmd.getConditions()));
    }
}
...
void MainWindow::refreshTableCommandes()
{
    ...
    ui->tableCommandes->setColumnCount(10);
    ui->tableCommandes->setHorizontalHeaderLabels({"ID", "Client", "Produit", "Quantité", "Statut", "Date", "Température", "Humidité", "Alerte", "Conditions"});
    ...
    for (int i = 0; i < list.size(); ++i) {
        ...
        ui->tableCommandes->setItem(i, 6, new QTableWidgetItem(QString::number(cmd.getTemperature(), 'f', 2)));
        ui->tableCommandes->setItem(i, 7, new QTableWidgetItem(QString::number(cmd.getHumidite(), 'f', 2)));
        ui->tableCommandes->setItem(i, 8, new QTableWidgetItem(cmd.getAlerte()));
        ui->tableCommandes->setItem(i, 9, new QTableWidgetItem(cmd.getConditions()));
    }
}
...
void MainWindow::setupMachineConnections()
{
    ...
    updateMachineToolControlsVisibility();
    QString testError;
    if (!MachineWidget::writeMachineLog(999, "TEST", "Test manuel du fichier log", &testError)) {
        qDebug() << "[TEST LOG] erreur =" << testError;
    } else {
        qDebug() << "[TEST LOG] fichier test cree avec succes";
    }
...
void MainWindow::updateMachineToolControlsVisibility()
{
    ...
    const bool showTools = ui->tabWidget_mach->currentWidget() == ui->tabListe_mach;
    ...
    if (ui->leftPanel) ui->leftPanel->setVisible(showTools);
    if (ui->splitter) {
        ui->splitter->setSizes(showTools ? QList<int>{260, 900} : QList<int>{0, 1160});
    }
}
...
void MainWindow::ajouterMachine()
{
    ...
    // Build machine data
    const QString categorie = ui->comboAddCategorie ? ui->comboAddCategorie->currentText() : "";
    const QString reference = ui->editAddReference ? ui->editAddReference->text().trimmed() : "";
    const QString etatAchat = ui->comboAddEtat ? ui->comboAddEtat->currentText() : "";
    const QString localisation = ui->editAddLocalisation ? ui->editAddLocalisation->text().trimmed() : "";
    ...
    // Validate and add
    QString error;
    const bool ajoutReussi = MachineWidget::addMachine(data, &error);
    if (!ajoutReussi) {
        QMessageBox::critical(this, "Erreur Machine", 
    ...
    if (ajoutReussi) {
        generateQRCodeForMachine(idMachine);
    }
...
void MainWindow::onSignalPanneMachine()
{
    ...
    QString typePanne = ui->comboTypePanne ? ui->comboTypePanne->currentText().trimmed() : "";
    QString gravite = ui->comboGravite ? ui->comboGravite->currentText().trimmed() : "";
    QString description = ui->txtDescriptionPanne ? ui->txtDescriptionPanne->toPlainText().trimmed() : "";
    QDate dateSignalement = ui->datePanne ? ui->datePanne->date() : QDate::currentDate();

    if (typePanne.isEmpty())
        typePanne = "Non specifie";

    if (gravite.isEmpty())
        gravite = "Non specifie";

    if (description.isEmpty())
        description = "Aucune description";

    // charger la machine depuis la base
    MachineData machine;
    QString error;
    if (!MachineWidget::getMachineById(idMachine, &machine, &error)) {
        QMessageBox::critical(this, "Erreur", "Impossible de charger la machine :\n" + error);
        return;
    }

    // changer l'etat en panne
    machine.etatAchat = "panne";

    int rowsAffected = 0;
    if (!MachineWidget::updateMachine(idMachine, machine, &rowsAffected, &error)) {
        QMessageBox::critical(this, "Erreur", "Impossible de signaler la panne :\n" + error);
        return;
    }

    // log clair de la panne
    QString detailsPanne = QString(
        "Type='%1', Gravite='%2', Date='%3', Description='%4'")
        .arg(typePanne)
        .arg(gravite)
        .arg(dateSignalement.toString("yyyy-MM-dd"))
        .arg(description);

    QString logError;
    if (!MachineWidget::writeMachineLog(idMachine, "PANNE_SIGNALEE", detailsPanne, &logError)) {
        QMessageBox::warning(this, "Log", "Panne signalee, mais erreur d'ecriture du log :\n" + logError);
    }

    QMessageBox::information(this, "Succes",
        QString("Panne signalee pour la machine \"%1\".").arg(machineName));

    afficherMachines();
    refreshMachineHistory();

    if (ui->txtDescriptionPanne)
        ui->txtDescriptionPanne->clear();

    if (ui->comboTypePanne)
...
void MainWindow::refreshMachineHistory()
{
    ...
    ui->tableHistoriqueMachines->clearContents();
    ...
    QStringList files = dir.entryList(QStringList() << "machine_*.txt", QDir::Files);
    ...
    for (const QString &fileName : files)
    {
        ...
        QString date, machineId, action, details;
        ...
        while (!in.atEnd())
        {
            ...
            if (line.startsWith("Date :"))
                ...
                machineId = line.mid(12).trimmed();

            else if (line.startsWith("Action :"))
                action = line.mid(8).trimmed();

            else if (line.startsWith("Details :"))
                details = line.mid(9).trimmed();

            else if (line.startsWith("===="))
            {
                if (!machineId.isEmpty())
                {
                    ui->tableHistoriqueMachines->insertRow(row);

                    ui->tableHistoriqueMachines->setItem(row, 0, new QTableWidgetItem(machineId));

                    QTableWidgetItem *actionItem = new QTableWidgetItem(action);
                    if (action == "AJOUT")
                        actionItem->setBackground(Qt::green);
                    else if (action == "MODIFICATION")
                        actionItem->setBackground(Qt::yellow);
                    else if (action == "SUPPRESSION")
                        actionItem->setBackground(Qt::red);
                    else if (action == "PANNE_SIGNALEE")
                        actionItem->setBackground(QColor(255, 165, 0)); // Orange
                    ui->tableHistoriqueMachines->setItem(row, 1, actionItem);

                    QString shortDetails = details.left(80) + "...";
                    QTableWidgetItem *item = new QTableWidgetItem(shortDetails);
                    item->setToolTip(details);
                    ui->tableHistoriqueMachines->setItem(row, 2, item);

                    ui->tableHistoriqueMachines->setItem(row, 3, new QTableWidgetItem(date));
...
void MainWindow::generateQRCodeForMachine(int idMachine)
{
    const QString text = "MACHINE_ID:" + QString::number(idMachine);

    // 👉 ICI tu mets la ligne
    //QDesktopServices::openUrl(QUrl(text));

    // (optionnel) afficher le lien
    // --- génération QR (tu peux la laisser) ---
    ...
    QDir projectDir(QStringLiteral(APP_PROJECT_SOURCE_DIR));
    const QString path = resolveMachineQrCodesPath();
    if (!projectDir.exists("qr_codes") && !projectDir.mkpath("qr_codes")) {
        QMessageBox::warning(this, "QR Code",
                             "Impossible de creer le dossier QR :\n" + path);
        return;
    }

    const QString file = QDir(path).filePath("machine_" + QString::number(idMachine) + ".png");

    if (!image.save(file, "PNG")) {
        QMessageBox::warning(this, "QR Code",
                             "Impossible d'enregistrer l'image QR dans :\n" + file);
        return;
    }

    qDebug() << "[Machine QR] saved to" << file;
    QMessageBox::information(this, "QR Code",
                             "Image QR enregistree avec succes dans :\n" + file);
}

void MainWindow::generateMachineHtmlFromLog(int idMachine)
{
    Q_UNUSED(idMachine);
    return;

    const QString logsPath = resolveMachineLogsPath();
    QString logPath = logsPath + "/machine_" + QString::number(idMachine) + ".txt";
    ...
    if (logFile.exists()) {
        if (logFile.open(QIODevice::ReadOnly | QIODevice::Text)) {
            ...
            logContent = "Le fichier log existe mais il est impossible de le lire.";
        }
    } else {
        logContent = "Aucun historique disponible pour cette machine.\n"
                     "Le fichier log n'existe pas encore.";
    }

    QString htmlPath = "C:/xampp/htdocs/projet/pages";
    QDir dir;
    if (!dir.exists(htmlPath)) {
        dir.mkpath(htmlPath);
    }

    QString filePath = htmlPath + "/machine_" + QString::number(idMachine) + ".html";

    QFile htmlFile(filePath);
    if (!htmlFile.open(QIODevice::WriteOnly | QIODevice::Text)) {
        QMessageBox::warning(this, "HTML", "Impossible de créer le fichier HTML.");
        return;
    }
    ...
    out << "  <title>Machine " << idMachine << "</title>\n";
    out << "  <style>\n";
    out << "    body { font-family: Arial, sans-serif; background:#f4f4f4; padding:30px; }\n";
    out << "    .card { background:white; max-width:900px; margin:auto; padding:25px; border-radius:12px; box-shadow:0 0 10px rgba(0,0,0,0.1); }\n";
    out << "    h1 { color:#2c3e50; }\n";
    out << "    pre { white-space: pre-wrap; word-wrap: break-word; font-size:16px; background:#fafafa; padding:15px; border-radius:8px; }\n";
    out << "  </style>\n";
    out << "</head>\n";
    out << "<body>\n";
    out << "  <div class='card'>\n";
    out << "    <h1>Fiche machine " << idMachine << "</h1>\n";
    out << "    <pre>" << logContent.toHtmlEscaped() << "</pre>\n";
    out << "  </div>\n";
    out << "</body>\n";
    out << "</html>\n";
...
void MainWindow::onGenerateMachineQRCode()
{
    int row = ui->tableMachines ? ui->tableMachines->currentRow() : -1;

    if (row < 0) {
        QMessageBox::warning(this, "QR Code", "Veuillez sélectionner une machine.");
        return;
    }

    QTableWidgetItem *idItem = ui->tableMachines->item(row, 0);
    if (!idItem) {
        QMessageBox::warning(this, "QR Code", "ID machine introuvable.");
        return;
    }

    int idMachine = idItem->text().toInt();
    if (idMachine <= 0) {
        QMessageBox::warning(this, "QR Code", "ID machine invalide.");
        return;
    }
...
void MainWindow::scanMachineQRCode()
{
    ...
    if (row < 0) {
        QMessageBox::warning(this, "Scanner QR Code", "Veuillez sélectionner une machine.");
        return;
    }

    QTableWidgetItem *idItem = ui->tableMachines->item(row, 0);
    if (!idItem) {
        QMessageBox::warning(this, "Scanner QR Code", "ID machine introuvable.");
        return;
    }

    int idMachine = idItem->text().toInt();
    if (idMachine <= 0) {
        QMessageBox::warning(this, "Scanner QR Code", "ID machine invalide.");
        return;
    }
...
// Ajouter cet include en haut du fichier
#include "arduino_environment.h"

void MainWindow::setupEnvironmentArduino()
{
    qDebug() << "[EnvArduino] ===== Setting up environment sensor =====";
    
    // ===== SEARCH COMMANDE BY ID (OPTIONAL - UI elements may not exist yet) =====
    // TODO: Add to Qt Designer when ready
    /*
    if (ui->inputArduinoCommandeId) {
        ui->inputArduinoCommandeId->setPlaceholderText("Entrez ID commande...");
        qDebug() << "[EnvArduino] ✓ Commande ID input found";
    } else {
        qWarning() << "[EnvArduino] ✗ inputArduinoCommandeId NOT FOUND!";
    }
    
    if (ui->btnSearchArduinoCommande) {
        connect(ui->btnSearchArduinoCommande, &QPushButton::clicked, this, &MainWindow::onSearchArduinoCommande);
        qDebug() << "[EnvArduino] ✓ Search button connected";
    } else {
        qWarning() << "[EnvArduino] ✗ btnSearchArduinoCommande NOT FOUND!";
    }
    
    if (ui->btnRecordArduinoData) {
        connect(ui->btnRecordArduinoData, &QPushButton::clicked, this, &MainWindow::onRecordArduinoData);
        qDebug() << "[EnvArduino] ✓ Record data button connected";
    } else {
        qWarning() << "[EnvArduino] ✗ btnRecordArduinoData NOT FOUND!";
    }
    */
    
    // ===== STATUS LABELS =====
    if (!ui->labelTemp) qWarning() << "[EnvArduino] ✗ labelTemp NOT FOUND!";
    else qDebug() << "[EnvArduino] ✓ labelTemp found";
    
    if (!ui->labelHum) qWarning() << "[EnvArduino] ✗ labelHum NOT FOUND!";
    else qDebug() << "[EnvArduino] ✓ labelHum found";
    
    if (!ui->labelFan) qWarning() << "[EnvArduino] ✗ labelFan NOT FOUND!";
    else qDebug() << "[EnvArduino] ✓ labelFan found";
    
    if (!ui->labelAlerteArduino) qWarning() << "[EnvArduino] ✗ labelAlerteArduino NOT FOUND!";
    else qDebug() << "[EnvArduino] ✓ labelAlerteArduino found";

    // ===== SETUP TABLE WIDGET =====
    if (ui->tableArduinoHistory) {
        ui->tableArduinoHistory->setColumnCount(5);
        ui->tableArduinoHistory->setHorizontalHeaderLabels(
            QStringList() << "Timestamp" << "Temp (°C)" << "Humidity (%)" << "Fan Event" << "Alert"
        );
        
        ui->tableArduinoHistory->horizontalHeader()->setSectionResizeMode(0, QHeaderView::Stretch);
        ui->tableArduinoHistory->horizontalHeader()->setSectionResizeMode(1, QHeaderView::ResizeToContents);
        ui->tableArduinoHistory->horizontalHeader()->setSectionResizeMode(2, QHeaderView::ResizeToContents);
        ui->tableArduinoHistory->horizontalHeader()->setSectionResizeMode(3, QHeaderView::Stretch);
        ui->tableArduinoHistory->horizontalHeader()->setSectionResizeMode(4, QHeaderView::ResizeToContents);
        
        qDebug() << "[EnvArduino] ✓ History table configured";
    } else {
        qWarning() << "[EnvArduino] ✗ tableArduinoHistory NOT FOUND!";
    }

    // ===== CONNECT ARDUINO BUTTONS =====
    if (ui->btnConnectEnvArduino) {
        connect(ui->btnConnectEnvArduino, &QPushButton::clicked, this, &MainWindow::onConnectEnvArduinoClicked);
        qDebug() << "[EnvArduino] ✓ Connect button connected";
    } else {
        qWarning() << "[EnvArduino] ✗ btnConnectEnvArduino NOT FOUND!";
    }
    
    if (ui->btnDisconnectEnvArduino) {
        connect(ui->btnDisconnectEnvArduino, &QPushButton::clicked, this, &MainWindow::onDisconnectEnvArduinoClicked);
        qDebug() << "[EnvArduino] ✓ Disconnect button connected";
    } else {
        qWarning() << "[EnvArduino] ✗ btnDisconnectEnvArduino NOT FOUND!";
    }

    // ===== NEW: SEARCH & RECORD BUTTONS =====
    if (ui->btnSearchCommande) {
        connect(ui->btnSearchCommande, &QPushButton::clicked, this, &MainWindow::onSearchArduinoCommande);
        qDebug() << "[EnvArduino] ✓ Search button connected";
    } else {
        qWarning() << "[EnvArduino] ✗ btnSearchCommande NOT FOUND!";
    }

    if (ui->btnRecordCommandeData) {
        connect(ui->btnRecordCommandeData, &QPushButton::clicked, this, &MainWindow::onRecordArduinoData);
        qDebug() << "[EnvArduino] ✓ Record button connected";
    } else {
        qWarning() << "[EnvArduino] ✗ btnRecordCommandeData NOT FOUND!";
    }
    
    if (ui->labelEnvArduinoStatus) {
        ui->labelEnvArduinoStatus->setText("Status: Disconnected");
        ui->labelEnvArduinoStatus->setStyleSheet("font-size:12px; font-weight:bold; color:#d32f2f;");
        qDebug() << "[EnvArduino] ✓ Status label initialized";
    } else {
        qWarning() << "[EnvArduino] ✗ labelEnvArduinoStatus NOT FOUND!";
    }
    
    qDebug() << "[EnvArduino] ===== Setup complete =====";
}

// ============================================================================
// SLOT: Search Commande by ID (OPTIONAL - uncomment when UI elements added)
// ============================================================================

void MainWindow::onSearchArduinoCommande()
{
    if (!ui->lineEditCommandeId) return;
    
    int commandeId = ui->lineEditCommandeId->text().toInt();
    if (commandeId <= 0) {
        if (ui->labelCommandeStatus) {
            ui->labelCommandeStatus->setText("❌ ID invalide");
            ui->labelCommandeStatus->setStyleSheet("color:#D32F2F;");
        }
        QMessageBox::warning(this, "Erreur", "Entrez un ID commande valide (> 0)");
        return;
    }
    
    // Search commande in database
    Ccommande cmd = Ccommande::getById(commandeId);
    if (cmd.getIdCommande() == 0) {
        if (ui->labelCommandeStatus) {
            ui->labelCommandeStatus->setText(QString("❌ Commande %1 non trouvée").arg(commandeId));
            ui->labelCommandeStatus->setStyleSheet("color:#D32F2F;");
        }
        QMessageBox::warning(this, "Erreur", QString("Commande ID %1 non trouvée").arg(commandeId));
        return;
    }
    
    // Store ID and set fixed threshold (28°C)
    m_currentArduinoCommandeId = commandeId;
    m_envArduino.setTargetTemperature(28.0f);  // ✅ Always 28°C
    
    // Show success message
    if (ui->labelCommandeStatus) {
        QString status = QString("✅ %1 (Seuil: 28.0°C)")
            .arg(cmd.getLibelle());
        ui->labelCommandeStatus->setText(status);
        ui->labelCommandeStatus->setStyleSheet("color:#2E7D32;");
    }
    
    QMessageBox::information(this, "Succès", 
        QString("✅ Commande trouvée:\nID: %1\nLibellé: %2\nSeuil: 28.0°C")
        .arg(commandeId).arg(cmd.getLibelle())
    );
    qDebug() << "[EnvArduino] Commande" << commandeId << "sélectionnée avec seuil 28°C";
}

// ============================================================================
// SLOT: Record Sensor Data
// ============================================================================

void MainWindow::onRecordArduinoData()
{
    qDebug() << "\n[MW] ==========================================";
    qDebug() << "[MW] onRecordArduinoData() CALLED";
    qDebug() << "[MW] m_currentArduinoCommandeId =" << m_currentArduinoCommandeId;
    qDebug() << "[MW] ==========================================\n";
    
    if (m_currentArduinoCommandeId <= 0) {
        QMessageBox::warning(this, "Erreur", "Veuillez d'abord rechercher une commande");
        qDebug() << "[MW] ❌ ID commande invalide:" << m_currentArduinoCommandeId;
        return;
    }
    
    if (!m_envArduino.isConnected()) {
        QMessageBox::warning(this, "Erreur", "Arduino non connecté");
        qDebug() << "[MW] ❌ Arduino pas connecté";
        return;
    }
    ...
    if (!m_envArduino.readSensorData(&data, &error)) {
        QMessageBox::critical(this, "Erreur", "Impossible de lire les données: " + error);
        qDebug() << "[MW] ❌ Erreur lecture Arduino:" << error;
        return;
    }
    
    qDebug() << "[MW] ✓ Données lues:";
    qDebug() << "[MW]   Temp:" << data.temperature << "°C";
    qDebug() << "[MW]   Humidity:" << data.humidity << "%";
    qDebug() << "[MW]   Fan RPM:" << data.fanRPM;
    
    // ✅ APPEL CRITIQUE : Utiliser la nouvelle méthode pour enregistrer UNE SEULE commande
    qDebug() << "[MW] 📝 Enregistrement pour COMMANDE ID =" << m_currentArduinoCommandeId;
    
    if (m_envArduino.recordSensorDataForSingleCommande(m_currentArduinoCommandeId, data, &error)) {
        QString msg = QString(
            "✅ Données enregistrées pour COMMANDE %1 UNIQUEMENT:\n"
            "Temp: %2°C\nHumidité: %3%\nFan: %4 RPM (%5)\n"
            "Alerte: %6"
        ).arg(m_currentArduinoCommandeId)
        ...
        qDebug() << "[MW] ✅ SUCCÈS: Commande" << m_currentArduinoCommandeId << "mise à jour";
    } else {
        QString fullError = error.isEmpty() ? "Erreur inconnue" : error;
        QMessageBox::critical(this, "Erreur", "Impossible d'enregistrer les données: " + fullError);
        qDebug() << "[MW] ❌ ÉCHEC: Commande" << m_currentArduinoCommandeId << "- Erreur:" << fullError;
    }
}
void MainWindow::startEnvironmentMonitoring()
{
    qDebug() << "[EnvArduino] startEnvironmentMonitoring called";
    
    if (m_envSensorTimer) {
        delete m_envSensorTimer;
    }
    
    m_envSensorTimer = new QTimer(this);
    m_envMonitorActive = true;
    
    // Connexion pour la lecture périodique
    connect(m_envSensorTimer, &QTimer::timeout, this, &MainWindow::readEnvironmentSensor);
    
    // Also read from serial readyRead for immediate data
    if (m_envArduino.getSerial()) {
        connect(m_envArduino.getSerial(), &QSerialPort::readyRead, this, [this]() {
            if (m_envMonitorActive) {
                readEnvironmentSensor();
            }
        });
        qDebug() << "[EnvArduino] Connected to serial readyRead signal";
    }
    
    // Lire toutes les 5 secondes
    m_envSensorTimer->start(5000);
    
    qDebug() << "[EnvArduino] Monitoring started (every 5 seconds)";
    
    // Force a first read immediately
    QTimer::singleShot(500, this, [this]() {
        readEnvironmentSensor();
    });
}

void MainWindow::readEnvironmentSensor()
{
    if (!m_envArduino.isConnected()) {
        qDebug() << "[EnvArduino] Not connected, attempting reconnect...";
        if (m_envArduino.connect() != 0) {
            qDebug() << "[EnvArduino] Reconnect failed";
            return;
        }
        qDebug() << "[EnvArduino] Reconnected successfully!";
    }
    ...
    if (m_envArduino.readSensorData(&data, &error)) {
        qDebug() << "[EnvArduino] READ SUCCESS - Temp:" << data.temperature 
                 << "°C | Hum:" << data.humidity 
                 << "% | RPM:" << data.fanRPM
                 << "| Fan:" << (data.fanRunning ? "ON" : "OFF")
                 << "| Valid:" << data.isValid;
        
        // Traiter les données reçues
        onEnvSensorDataReceived(data);
    } else {
        qDebug() << "[EnvArduino] Read error:" << error;
    }
}
void MainWindow::onEnvSensorDataReceived(const EnvSensorData &data)
{
    m_lastEnvData = data;

    // ─── Calcul de l'alerte qualite ───────────────────────────────────────
    QString alert = "NORMAL";
    float threshold = m_envArduino.getTargetTemperature();
    if (data.temperature > threshold + 10.0) {
        alert = "CRITICAL";
    } else if (data.temperature > threshold) {
        alert = "WARNING";
    }
    if (data.humidity > 75.0f) {
        alert = (alert == "CRITICAL") ? "CRITICAL" : "WARNING";
    }

    // ─── Construire le message pour l'historique ───────────────────────────
    bool shouldFanRun = (data.temperature >= threshold);
    QString fanEvent = shouldFanRun ? "📍 RUNNING" : "📍 OFF";

    // ─── TOUJOURS AJOUTER A L'HISTORIQUE (chaque lecture) ────────────────
    addArduinoHistoryRow(data, fanEvent, alert);

    // ─── Mise a jour des labels UI ────────────────────────────────────────
    if (ui->labelTemp) {
        ui->labelTemp->setText(QString::number(data.temperature, 'f', 1) + " °C");
    }
    if (ui->labelHum) {
        ui->labelHum->setText(QString::number(data.humidity, 'f', 1) + " %");
    }
    if (ui->labelFan) {
        bool fanOn = shouldFanRun || data.fanRunning;
        QString fanText = fanOn
            ? QString("🔥 EN MARCHE (%1 RPM)").arg(data.fanRPM)
            : QString("❄️ ARRETE");
        ui->labelFan->setText(fanText);
        ui->labelFan->setStyleSheet(fanOn
            ? "font-size:14px; font-weight:bold; color:#2E7D32;"
            : "font-size:14px; font-weight:bold; color:#757575;");
    }
    if (ui->labelAlerteArduino) {
        applyFanAlertStyle(alert);
    }

    // ─── Sauvegarde en base de donnees ────────────────────────────────────
    saveArduinoReadingToDB(data, alert);

    // ─── Alerte globale dans la barre de statut ───────────────────────────
    if (alert == "CRITICAL") {
        if (ui->statusbar1)
            ui->statusbar1->showMessage(
                QString("🚨 ALERTE CRITIQUE: Temperature %1°C - Ventilateur force!").arg(data.temperature, 0, 'f', 1), 6000);
    } else if (alert == "WARNING") {
        if (ui->statusbar1)
            ui->statusbar1->showMessage(
                QString("⚠️ Attention: Temperature %1°C depasse le seuil %2°C").arg(data.temperature, 0, 'f', 1).arg(m_envArduino.getTargetTemperature(), 0, 'f', 1), 4000);
    }

    // ─── Mise a jour DB commandes et labos ────────────────────────────────
    updateCommandesWithEnvironmentData();
    updateLabosWithEnvironmentData();
}

void MainWindow::updateCommandesWithEnvironmentData()
{
    const EnvSensorData &sd = m_lastEnvData;
    if (!sd.isValid) return;

    // ✅ Si aucune commande spécifique n'est recherchée, ne rien faire
    if (m_currentArduinoCommandeId <= 0) {
        qDebug() << "[EnvArduino] ⏭️  Monitoring: Aucune commande recherchée - mise à jour IGNORÉE";
        return;
    }

    // Calcul alerte based on user-configured threshold
    QString alert = "NORMAL";
    float threshold = m_envArduino.getTargetTemperature();
    if (sd.temperature > threshold + 10.0) alert = "CRITICAL";
    else if (sd.temperature > threshold)   alert = "WARNING";

    // storageConditions resume
    QString conditions = QString("T=%1C H=%2% Fan=%3")
        .arg(sd.temperature, 0, 'f', 1)
        .arg(sd.humidity,    0, 'f', 1)
        .arg(sd.fanRunning ? "ON" : "OFF");
    ...
    query.prepare(
        ...
        "QUALITY_ALERT       = :alert, "
        "STORAGE_CONDITIONS  = :cond "
        "WHERE ID_COMMANDE = :id"
    );
    query.bindValue(":temp",  sd.temperature);
    query.bindValue(":hum",   sd.humidity);
    query.bindValue(":alert", alert);
    query.bindValue(":cond",  conditions);
    query.bindValue(":id",    m_currentArduinoCommandeId);

    if (query.exec()) {
        int updated = query.numRowsAffected();
        qDebug() << "[EnvArduino] 📊 Monitoring update: Commande ID" << m_currentArduinoCommandeId 
                 << "mise à jour -" << updated << "ligne(s), alert=" << alert;
    } else {
        qDebug() << "[EnvArduino] ❌ Commande update error:" << query.lastError().text();
    }
}

void MainWindow::updateLabosWithEnvironmentData()
{
    const EnvSensorData &sd = m_lastEnvData;
    if (!sd.isValid) return;

    // ✅ ONLY update if a specific labo is being monitored
    // This prevents bulk updates to ALL laboratories
    // Currently there's no m_currentArduinoLaboId, so we skip this for now
    // In the future, this should filter to: WHERE ID_LABO = :id
    
    // For now, disable lab updates to prevent bulk modifications
    qDebug() << "[EnvArduino] Skipping labo update - no specific labo selected for monitoring";
    return;

    QString fanStatus = sd.fanRunning ? sd.fanMode : "OFF";
    ...
    query.prepare(
        ...
        "HUMIDITY_ENV       = :hum,  "
        "LAST_ENV_CHECK     = SYSDATE, "
        "VENTILATION_STATUS = :fan,  "
        "ENV_ALERT = CASE "
        "  WHEN :temp > 35 OR :hum > 75 THEN 'CRITICAL' "
        "  WHEN :temp > 30 OR :hum > 65 THEN 'WARNING' "
        "  ELSE 'NORMAL' END"
    );
    query.bindValue(":temp", sd.temperature);
    query.bindValue(":hum",  sd.humidity);
    query.bindValue(":fan",  fanStatus);

    if (query.exec()) {
        qDebug() << "[EnvArduino] Labos updated:" << query.numRowsAffected() << "rows";
        if (ui->stackedMain->currentWidget() == ui->LaboWidget) {
            afficherLabos();
        }
    } else {
        qDebug() << "[EnvArduino] Labo update error:" << query.lastError().text();
    }
}

void MainWindow::chargerMachinePourModification()
{
}

void MainWindow::onEnvArduinoCommandSent(bool success, const QString &message)
{
    if (ui->statusbar1) {
        ui->statusbar1->showMessage(success ? "Arduino: " + message : "Erreur Arduino: " + message, 4000);
    }
}

void MainWindow::onClearArduinoHistory()
{
    if (ui->tableArduinoHistory) {
        ui->tableArduinoHistory->setRowCount(0);
    }
}

void MainWindow::addArduinoHistoryRow(const EnvSensorData &data, const QString &fanEvent, const QString &alert)
{
    if (!ui->tableArduinoHistory) return;

    int row = ui->tableArduinoHistory->rowCount();
    ui->tableArduinoHistory->insertRow(0); // insert en haut (plus recent en premier)

    QString ts = QDateTime::currentDateTime().toString("dd/MM/yyyy HH:mm:ss");
    ...
    ui->tableArduinoHistory->setItem(0, 0, makeItem(ts));
    ui->tableArduinoHistory->setItem(0, 1, makeItem(
        data.isValid ? QString::number(data.temperature, 'f', 1) : "--"));
    ui->tableArduinoHistory->setItem(0, 2, makeItem(
        data.isValid ? QString::number(data.humidity,    'f', 1) : "--"));
    ui->tableArduinoHistory->setItem(0, 3, makeItem(fanEvent));
    ui->tableArduinoHistory->setItem(0, 4, makeItem(alert));

    // Colorisation selon alerte
    QColor rowColor = (alert == "CRITICAL") ? QColor("#FFCDD2")
                    : (alert == "WARNING")  ? QColor("#FFF9C4")
                    : QColor("#E8F5E9");
    for (int col = 0; col < 5; ++col) {
        if (auto *item = ui->tableArduinoHistory->item(0, col)) {
            item->setBackground(rowColor);
        }
    }

    // Garder max 200 lignes
    while (ui->tableArduinoHistory->rowCount() > 200) {
        ui->tableArduinoHistory->removeRow(ui->tableArduinoHistory->rowCount() - 1);
    }
    Q_UNUSED(row);
}

void MainWindow::saveArduinoReadingToDB(const EnvSensorData &data, const QString &alert)
{
    ...
    QString conditions = QString("T=%1C H=%2% Fan=%3")
        .arg(data.temperature, 0, 'f', 1)
        .arg(data.humidity,    0, 'f', 1)
        .arg(data.fanRunning ? "ON" : "OFF");

    // ✅ ONLY update if a specific commande is being monitored
    // Prevents bulk updates to ALL active commandes
    if (m_currentArduinoCommandeId <= 0) {
        qDebug() << "[Arduino DB] ⏭️  COMMANDE save skipped - no commande selected for monitoring";
        return;  // Don't update any commandes if none selected
    }
    ...
    query.prepare(
        ...
        "STORAGE_CONDITIONS  = :cond, "
        "LAST_SENSOR_READING = SYSDATE "
        "WHERE ID_COMMANDE = :id"  // ✅ CHANGED: Only specific commande
    );
    query.bindValue(":temp",  data.temperature);
    query.bindValue(":hum",   data.humidity);
    query.bindValue(":alert", alert);
    query.bindValue(":cond",  conditions);
    query.bindValue(":id",    m_currentArduinoCommandeId);  // ✅ ADDED
    
    if (!query.exec()) {
        qDebug() << "[Arduino DB] COMMANDE update error:" << query.lastError().text();
    } else {
        qDebug() << "[Arduino DB] ✅ Commande" << m_currentArduinoCommandeId 
                 << "updated:" << query.numRowsAffected() << "rows";
    }

    // ✅ Disable labo updates for now (same bulk update issue)
    qDebug() << "[Arduino DB] ⏭️  LABO update skipped - monitoring disabled to prevent bulk updates";
}

void MainWindow::applyFanAlertStyle(const QString &alert)
{
    if (!ui->labelAlerteArduino) return;
    ui->labelAlerteArduino->setText(alert);
    if (alert == "CRITICAL") {
        ui->labelAlerteArduino->setStyleSheet("font-size:14px; font-weight:bold; color:#B71C1C;");
    } else if (alert == "WARNING") {
        ui->labelAlerteArduino->setStyleSheet("font-size:14px; font-weight:bold; color:#E65100;");
    } else {
        ui->labelAlerteArduino->setStyleSheet("font-size:14px; font-weight:bold; color:#2E7D32;");
    }
}
void MainWindow::stopEnvironmentMonitoring()
{
    qDebug() << "[EnvArduino] Stopping environment monitoring...";
    m_envMonitorActive = false;
    if (m_envSensorTimer) {
        m_envSensorTimer->stop();
        qDebug() << "[EnvArduino] Timer stopped";
    }
}

// ============================================================================
// NEW: Arduino Environment Manual Connection Handlers
// ============================================================================

void MainWindow::onConnectEnvArduinoClicked()
{
    qDebug() << "[EnvArduino] Connect button clicked";
    
    // Mutual exclusion: Disconnect Fingerprint Arduino if connected
    if (A.getserial() && A.getserial()->isOpen()) {
        A.close_arduino();
        m_fingerprintSerial = nullptr; // Reset the pointer
        qDebug() << "[EnvArduino] Disconnected Fingerprint Arduino for mutual exclusion";
        
        clearFingerprintOperationState();
        if (m_fingerprintAuthDialog) {
            m_fingerprintAuthDialog->reject();
        }

        if (ui->statusbar1) {
            ui->statusbar1->showMessage("Capteur d'empreinte deconnecte pour utiliser l'environnement", 5000);
        }
    }

    // Try to connect
    int ret = m_envArduino.connect();
    qDebug() << "[EnvArduino] Connection result:" << ret;
    
    switch (ret) {
    case 0:
        qDebug() << "[EnvArduino] ✓ Connected successfully on port:" << m_envArduino.getPortName();
        updateEnvArduinoStatus();
        if (ui->statusbar1) {
            ui->statusbar1->showMessage("✓ Capteur environnement connecté sur " + m_envArduino.getPortName(), 5000);
        }
        startEnvironmentMonitoring();
        break;
    case 1:
        qWarning() << "[EnvArduino] ✗ Port found but connection failed";
        updateEnvArduinoStatus();
        if (ui->statusbar1) {
            ui->statusbar1->showMessage("⚠ Arduino détecté mais connexion échouée", 5000);
        }
        break;
    default:
        qWarning() << "[EnvArduino] ✗ Arduino not found";
        updateEnvArduinoStatus();
        if (ui->statusbar1) {
            ui->statusbar1->showMessage("✗ Arduino non trouvé sur les ports sériels", 5000);
        }
    }
}

void MainWindow::onDisconnectEnvArduinoClicked()
{
    qDebug() << "[EnvArduino] Disconnect button clicked";
    ...
    if (ui->statusbar1) {
        ui->statusbar1->showMessage("✓ Capteur environnement déconnecté", 3000);
    }
}

void MainWindow::updateEnvArduinoStatus()
{
    if (!ui->labelEnvArduinoStatus) return;
    
    if (m_envArduino.isConnected()) {
        ui->labelEnvArduinoStatus->setText("Status: Connected (" + m_envArduino.getPortName() + ")");
        ui->labelEnvArduinoStatus->setStyleSheet("font-size:12px; font-weight:bold; color:#2E7D32;");
        if (ui->btnConnectEnvArduino) ui->btnConnectEnvArduino->setEnabled(false);
        if (ui->btnDisconnectEnvArduino) ui->btnDisconnectEnvArduino->setEnabled(true);
    } else {
        ui->labelEnvArduinoStatus->setText("Status: Disconnected");
        ui->labelEnvArduinoStatus->setStyleSheet("font-size:12px; font-weight:bold; color:#d32f2f;");
        if (ui->btnConnectEnvArduino) ui->btnConnectEnvArduino->setEnabled(true);
        if (ui->btnDisconnectEnvArduino) ui->btnDisconnectEnvArduino->setEnabled(false);
    }
}
...
void MainWindow::closeEvent(QCloseEvent *event)
{
    ...
    if (A.getserial() && A.getserial()->isOpen()) {
        A.close_arduino();
        msg += "Carte Arduino Empreinte deconnectee avec succes.\n";
        showedMessage = true;
    }

    if (m_envArduino.isConnected()) {
        m_envArduino.disconnect();
        msg += "Carte Arduino Environnement deconnectee avec succes.\n";
        showedMessage = true;
    }

    if (showedMessage) {
        QMessageBox::information(this, "Fermeture de l'application", msg.trimmed());
    }
