...
bool LaboRepository::getSpaceUsageStats(QList<LaboSpaceUsage> *outStats, QString *errorMessage)
{
    if (!outStats) {
        return false;
    }
    ...
    QSqlQuery query;
    if (!query.exec(
            "SELECT NOM_LABO, ESPACE_LABO, ESPACE_RESTE "
            "FROM LABO WHERE ESPACE_LABO > 0 ORDER BY NOM_LABO")) {
        if (errorMessage) {
            *errorMessage = query.lastError().text();
        }
        return false;
    ...
    while (query.next()) {
        LaboSpaceUsage stat;
        stat.nomLabo = query.value("NOM_LABO").toString();
        int espaceLabo = query.value("ESPACE_LABO").toInt();
        int espaceReste = query.value("ESPACE_RESTE").toInt();
        if (espaceLabo > 0) {
            stat.percentageUsed = ((double)(espaceLabo - espaceReste) / espaceLabo) * 100.0;
        }
...
bool LaboRepository::getConformiteStats(QList<LaboConformiteCount> *outStats, QString *errorMessage)
{
    if (!outStats) {
        return false;
    }
    ...
    if (!query.exec(
            ...
            "FROM LABO GROUP BY CONFORMITE ORDER BY CONFORMITE")) {
        if (errorMessage) {
            *errorMessage = query.lastError().text();
        }
        return false;
    ...
    while (query.next()) {
        LaboConformiteCount stat;
        stat.conformite = query.value("CONFORMITE").toString();
...
bool LaboRepository::getReviewStats(LaboReviewStats *outStats, QString *errorMessage)
{
    if (!outStats) {
        return false;
    }
    ...
    QSqlQuery query;
    if (!query.exec(
            "SELECT AVG(REVIEW) as avg_review, COUNT(REVIEW) as total_reviews "
            "FROM LABO WHERE REVIEW BETWEEN 0 AND 10")) {
        if (errorMessage) {
            *errorMessage = query.lastError().text();
        }
        return false;
    ...
    if (query.next()) {
        outStats->averageReview = query.value("avg_review").toDouble();
        outStats->totalReviews = query.value("total_reviews").toInt();
    }
    return true;
}