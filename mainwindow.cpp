...
{
    // Load from DB via Client model
    refreshTableFromDB();

    // Populate filter/form combos (only if empty)
...
{
    // SELECT * column order from DB:
    // 0=ID_CLIENT, 1=NOM_CLIENT, 2=T?L?PHONE(accented), 3=EMAIL,
    // 4=ADRESSE, 5=TYPE_CLIENT, 6=STATUT, 7=ID_EMPLOYE
    // Table display order: 0=ID, 1=Nom, 2=Type, 3=Telephone, 4=Email, 5=Statut
    ...
    for (int i = 0; i < model->rowCount(); ++i) {
        ...
        // Map DB columns to table display columns
        ui->tableClients->setItem(row, 0, new QTableWidgetItem(
            model->data(model->index(i, 0)).toString())); // ID_CLIENT
        ui->tableClients->setItem(row, 1, new QTableWidgetItem(
            model->data(model->index(i, 1)).toString())); // NOM_CLIENT
        ui->tableClients->setItem(row, 2, new QTableWidgetItem(
            model->data(model->index(i, 5)).toString())); // TYPE_CLIENT
        ui->tableClients->setItem(row, 3, new QTableWidgetItem(
            model->data(model->index(i, 2)).toString())); // T?L?PHONE (by index)
        ui->tableClients->setItem(row, 4, new QTableWidgetItem(
            model->data(model->index(i, 3)).toString())); // EMAIL
        ui->tableClients->setItem(row, 5, new QTableWidgetItem(
            model->data(model->index(i, 6)).toString())); // STATUT
    }
    delete model;
}
...
void MainWindow::onConfirmerAjouterClicked()
{
    ...
    // Validation metier — toutes les regles sont dans Client::valider()
    QString erreur;
    ...
    // Appel vers la classe metier
    Client c(0, nom, type, tel, email, statut);

    QString dbErreur;
    if (c.ajouter(&dbErreur)) {
        // Succes: rafraichir table + message succes
        refreshTableFromDB();
        QMessageBox::information(this, "Succès", "Client ajouté avec succès !");
        clearFormAjouter();
        ui->tabWidgetClient->setCurrentWidget(ui->tabListeClients);
    } else {
        // Sinon: afficher errorMessage
        QMessageBox::critical(this, "Erreur",
            "Échec de l'ajout du client dans la base de données.\n\n" + dbErreur);
    }
}
...
void MainWindow::onConfirmerSupprimerClicked()
{
    ...
    if (reply == QMessageBox::Yes) {
        // Appel vers la classe métier
        if (m_clientTmp.supprimer(id)) {
            // Succès: rafraîchir table + message
            refreshTableFromDB();
            clearClientInfo();
...
void MainWindow::onAppliquerFiltreClicked()
{
    ...
    for (int i = 0; i < ui->tableClients->rowCount(); ++i) {
        ...
        if (typeFiltre != "Tous") {
            if (ui->tableClients->item(i, 2)->text() != typeFiltre)
                visible = false;
        }
        if (statutFiltre != "Tous") {
            if (ui->tableClients->item(i, 5)->text() != statutFiltre)
                visible = false;
        }
...
static QString buildListeClientsHTML(QTableWidget *table)
{
    ...
    html += "<div class='footer'>Smart Oil Press &#8212; " +
...
static QString buildStatistiquesHTML(QTableWidget *table)
{
    ...
    html += "<h2>Vue d'ensemble</h2><table>"
            ...
            "<tr><td>Total Clients</td><td><b>" + QString::number(total) + "</b></td></tr>"
            "<tr><td>Clients Actifs</td><td>" + QString::number(actif) + "</td></tr>"
            "<tr><td>Clients Inactifs</td><td>" + QString::number(inactif) + "</td></tr>"
            "<tr><td>En attente</td><td>" + QString::number(attente) + "</td></tr>"
            "</table>";
    int pctP = total > 0 ? (particulier * 100 / total) : 0;
    int pctE = total > 0 ? (entreprise * 100 / total) : 0;
    html += "<h2>R&#233;partition par type</h2><table>"
            ...
            "<tr><td>Entreprises</td><td>" + QString::number(entreprise) + "</td><td>" + QString::number(pctE) + "%</td></tr>"
            "</table>";
    html += "<div class='footer'>Smart Oil Press &#8212; " +
            QDateTime::currentDateTime().toString("yyyy") + "</div></body></html>";
    return html;
...
static void saveClientPDF(QWidget *parent, const QString &html,
                          const QString &defaultName, QTextEdit *msgBox)
{
    ...
    if (msgBox)
        msgBox->append("&#10003; PDF export&#233; : " + fileName);
...
void MainWindow::onExportFicheClientClicked()
{
    ...
    if (id.isEmpty()) {
        QMessageBox::warning(this, "ID manquant",
            "Veuillez saisir un ID client dans le champ d'export.");
        return;
    }
